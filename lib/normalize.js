...
function normalize(rootSchema, rootValues, options) {
	...
	var after = options.after || function (schema, resolved) { return resolved; };

	return (_instance(extend(rootSchema), rootValues) || resolve({}))();

	function _instance(schema, values) {
		return after(schema, before(schema, values) || _array(schema, values) || _object(schema, values) || _primary(schema, values) || _primitive(schema, values));
	}
	...
	function _array(schema, values) {
		...
		function _loose() {
			if (Array.isArray(values) && (!schema.type || _.includes(schema.type, 'array'))) {
				return values;
			}
		}
	...
	function _object(schema, values) {
		...
		function _properties() {
			...
			function _property(propertySchema, property) {
				...
				function _default() {
					...
					function __required() {
						return schema.required && _.includes(schema.required, property);
					}
		...
		function _gathering() {
			...
			// TODO: normalize gathered values if the "gathering" property is defined in "properties".
