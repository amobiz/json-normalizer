	...
	function _async() {
		deref(schema, options || {}, function (err, derefSchema) {
			...
			if (err) {
				...
				errors = [];
				result = normalize(derefSchema, values, errors, options);
				if (errors.length > 0) {
					callback(errors);
				} else {
...
function sync(schema, values, optionalOptions) {
	...
	result = {
		values: normalize(derefSchema, values, errors, options)
	};
	if (errors.length) {
...
function normalize(rootSchema, rootValues, errors, options) {
	return (_instance(rootSchema, rootValues) || resolve(rootValues))();

	function _instance(schema, values) {
		var extendedSchema;

		extendedSchema = extend(schema);
		return _array(extendedSchema, values) || _object(extendedSchema, values) || _primary(extendedSchema, values) || _primitive(extendedSchema, values);
	}

	function _array(schema, values) {
		// note: if schema.type is 'array, we force value to be an array.
		if (_strict() || _loose()) {
			return suffice(resolve(reduce(values, _item, [])));
		}
		...
		function _loose() {
			return Array.isArray(values) && (!schema.type || _.contains(schema.type, 'array'));
		}
		...
		function _item(ret, value) {
			...
			return _accumulate(suffice(_instance(itemSchema, value)));
